        ...
        <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="ease-in duration-200" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
          <DialogPanel class="mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
            <Combobox @update:modelValue="onSelect">
              <div class="relative">
                <Icon name="heroicons:magnifying-glass-20-solid" class="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-gray-400" aria-hidden="true" />
                  ...
                  <ul class="-mx-4 mt-2 text-sm text-gray-700">
                    <ComboboxOption v-for="project in filteredProjects" :key="project.id" v-slot="{ active }" :value="project" as="template">
                      <li class="flex cursor-default select-none items-center px-4 py-2" :class="[active && 'bg-indigo-600 text-white']">
                        <Icon name="heroicons:folder" class="h-6 w-6 flex-none" :class="[active ? 'text-white' : 'text-gray-400']" aria-hidden="true" />
              ...
              <div v-if="rawQuery === '?'" class="py-14 px-6 text-center text-sm sm:px-14">
                <Icon name="heroicons:lifebuoy" class="mx-auto h-6 w-6 text-gray-400" aria-hidden="true" />
              ...
              <div v-if="query !== '' && rawQuery !== '?' && filteredProjects.length === 0 && filteredUsers.length === 0" class="py-14 px-6 text-center text-sm sm:px-14">
                <Icon name="heroicons:exclamation-triangle" class="mx-auto h-6 w-6 text-gray-400" aria-hidden="true" />
                <p class="mt-4 font-semibold text-gray-900">
                  No results found
                </p>
                <p class="mt-2 text-gray-500">
